...
import sys
import time
import optparse  # for input parsing
import random
import numpy as np
import torch
...
import traci  # for interacting with sumo get state , applying action given by network
...
class Agent:
    ...
    def save(self, model_name):
        torch.save(self.Q_eval.state_dict(), f'model/{model_name}.bin')
...
def run(train=True, model_name="model", epochs=50, steps=500):
    ...
    traci.start(
        [checkBinary("sumo"), "-c", "configuration.sumocfg",
         "--tripinfo-output", "network/tripinfo.xml"]
    )
    ...
    if not train:
        brain.Q_eval.load_state_dict(torch.load(
            f'model/{model_name}.bin', map_location=brain.Q_eval.device))
        ...
        while step <= steps:
            ...
            for junction_number, junction in enumerate(all_junctions):
                ...
                if traffic_lights_time[junction] == 0:
                    ...
                    # storing previous state and current state into replay memory
    ...
    if train:
        ...
        plt.savefig(f'result/Average_Waiting_Time_vs_Epochs{model_name}.png')
        plt.show()
# run() function end here


# used to parser parameter passed from the terminal
def get_options():
